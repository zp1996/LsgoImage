...
(function (g) {
	...
	LSGOImage.fn.getInfo = function (canvas) {
		this.imgData = this.ctx.getImageData(this.startX, this.startY, this.width - this.startX, this.height - this.startY);
		this.oldData = [];
	...
	LSGOImage.fn.changeImage = function (arr) {
		...
		this.imgData.data.forEach(function (val, i) {
			_this.imgData.data[i] = arr[i];
		});
	...
	LSGOImage.fn.BaseRoberts = function (arr, fn) {
		var width = this.width,
		    ...
		    newY;
		for (var y = 0; y < height; y++) {
			var xgr, xgg, xgb, ygr, ygb, ygg, mr, mb, mg;
